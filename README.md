...
#### Sample command
```bash
sh run.sh -i 192.168.25.79 -r /AGX -c config/bridge.yaml # This will run the bridge with the specified robot IP, ROS topic prefix, and configuration file. (Port is set to 1883 by default, MQTT topic prefix is set to empty by default)
```